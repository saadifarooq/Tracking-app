...
def load_data():
    ...
    tracking_col = find_col(po.columns, ["Tracking Number", "Tracking#", "Tracking ID", "Tracking"])
    ...
    required = {
        "master SKU": master_sku_col,
        "master image URL": master_img_col,
        "PO SKU": po_sku_col,
        "PO tracking number": tracking_col,
    }
    ...
    for _, row in master.iterrows():
        ...
        if not sku:
            continue
        entry = master_lookup.setdefault(sku, {
            "sku": sku,
            "name": clean_text(row.get(master_name_col, "")) if master_name_col else "",
            "image_urls": [],
            "item_url": clean_text(row.get(master_item_url_col, "")) if master_item_url_col else "",
        })
        img = clean_text(row.get(master_img_col, ""))
        if img and img not in entry["image_urls"]:
            entry["image_urls"].append(img)
        if not entry["name"] and master_name_col:
            entry["name"] = clean_text(row.get(master_name_col, ""))
        if not entry["item_url"] and master_item_url_col:
    ...
    return {
        "records": records,
        "stats": {
            "po_rows_loaded": len(records),
            "master_skus_loaded": len(master_lookup),
...
def group_results(records):
    ...
    for rec in records:
        ...
        group = grouped.setdefault(tracking, {
            "tracking": tracking,
            "po": rec.get("po", ""),
            "order": rec.get("order", ""),
            "items": [],
        ...
        if not any((x["sku"], x.get("description", "")) == item_key for x in group["items"]):
            group["items"].append({
                "sku": rec["sku"],
                "qty": rec.get("qty", ""),
                "description": rec.get("description") or rec.get("master_name", ""),
...
def admin_upload():
    if request.method == "POST":
        ...
        try:
            # Validate that the file can be loaded and has required columns before switching employees to it.
            old_pointer = get_active_po_file()
            set_active_po_file(saved_path)
            data = load_data()
            flash(f"Daily order file uploaded successfully. Loaded {data['stats']['po_rows_loaded']} searchable row(s).", "success")
...
def api_search():
    ...
    if exact:
        return jsonify({"ok": True, "match_type": "exact", "query": q_raw, "results": group_results(exact), "stats": data["stats"]})
    ...
    if len(q_digits) >= MIN_PARTIAL_DIGITS:
        ...
        return jsonify({
            "ok": False,
            "message": f"For partial search, type at least {MIN_PARTIAL_DIGITS} digits, such as the last 7 or 8 digits.",
            "results": [],
            "stats": data["stats"],
        }), 400
